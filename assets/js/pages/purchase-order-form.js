(function () {
  ...
  function renderBatchPicker() {
    ...
    var rows = visible.length ? visible.map(function (product) {
      ...
      return '<tr data-purchase-picker-product="' + text(product.code) + '"><td><input type="checkbox" data-purchase-picker-check value="' + text(product.code) + '"' + (batchPickerState.selected.has(product.code) ? ' checked' : '') + (exists ? ' disabled' : '') + '></td><td>' + image + '</td><td class="product-picker-product" title="' + text(service.displayName(product)) + '">' + text(service.displayName(product)) + '</td><td>' + text(product.unit || '--') + '</td><td>' + fixed(product.purchasePrice || product.marketPrice || 0) + '</td><td><input type="number" min="0.01" step="0.01" data-purchase-picker-quantity value="' + text(draft.quantity || '') + '" placeholder="' + (exists ? '已添加' : '请输入数量') + '"' + (exists ? ' disabled' : '') + '></td><td><input type="text" data-purchase-picker-remark value="' + text(draft.remark || '') + '" placeholder="请输入备注"' + (exists ? ' disabled' : '') + '></td></tr>';
