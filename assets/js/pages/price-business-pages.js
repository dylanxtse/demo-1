(function () {
  ...
  function selectField(label, name, values, selected = '', first = '全部', cls = '') {
    ...
    return field(label, `<select class="filter-select" data-filter="${esc(name)}" aria-label="${esc(label)}"${placeholderData}>${emptyOption}${valueOptions}</select>`, cls);
  }
  ...
  function formDateRange(from = '', to = '', name = 'agreementExecutionCycle') {
    const displayId = `${name}Display`;
    return `<div class="date-range-picker operations-date-range pb-form-date-range"><input class="filter-input date-range-display" id="${displayId}" type="text" readonly placeholder="请选择" aria-label="执行周期"><input type="hidden" data-date-start data-form-field="from" value="${esc(from)}"><input type="hidden" data-date-end data-form-field="to" value="${esc(to)}"><span class="date-range-icon" aria-hidden="true"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg></span></div>`;
  }
  ...
  function bindDatePickers() {
    if (window.DateRangePicker?.create) {
      host.querySelectorAll('.date-range-picker').forEach((container) => {
        const picker = window.DateRangePicker.create({ container });
        if (picker) state.datePickers.push(picker);
      });
    }
    const selector = '[data-date-picker], [data-form-field="deadline"], [data-form-field="date"]';
  ...
  function formDefaults(type) {
    ...
    return { entity: '', entities: [], from: '', to: '', remark: '', contact: '', phone: '', lines: Array.from({ length: 10 }, () => ({ code: '', price: '' })) };
  }
