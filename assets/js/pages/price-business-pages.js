(function () {
  ...
  root.addEventListener('input', (event) => {
    ...
    if (event.target.matches('[data-form-field="remark"]')) { const counter = event.target.parentElement.querySelector('.pb-counter'); if (counter) counter.textContent = `${event.target.value.length}/${config.mode === 'sales' ? 50 : 20}`; }
  });
