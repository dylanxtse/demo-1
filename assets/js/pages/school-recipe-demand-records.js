(function () {
  ...
  const expectedAtValue = (record) => record?.expectedAt || record?.orders?.find((order) => order?.expectedAt)?.expectedAt || '';
  ...
  const state = { keyword: '', submittedDate: '', usageDate: '', expectedAtDate: '', page: 1, pageSize: 20, pagination: null };
  ...
  const datePickers = {
    ...
    expectedAt: window.DatePicker?.create?.({ input: page.querySelector('#schoolRecipeDemandRecordExpectedAt'), panelId: 'schoolRecipeDemandRecordExpectedAtPanel' })
  };
  ...
  function getFilteredRecords() {
    return service.getAll().filter((record) => {
      ...
      if (state.expectedAtDate && String(expectedAtValue(record)).slice(0, 10) !== state.expectedAtDate) return false;
      return true;
    });
  }
  ...
  function renderRows(records = getFilteredRecords()) {
    ...
    body.innerHTML = pageRecords.length ? pageRecords.map((record) => `<tr>
      ...
      <td>${escapeHtml(expectedAtValue(record) || '--')}</td>
  ...
  page.querySelector('#schoolRecipeDemandRecordsFilter').addEventListener('submit', (event) => {
    ...
    state.expectedAtDate = page.querySelector('#schoolRecipeDemandRecordExpectedAt').value || '';
    refresh(true);
  });
