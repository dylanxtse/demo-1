  ...
  function renderBidForm() {
    ...
    const form = `
      <div class="page-card bidding-form-page" id="bidFormPage">
        <div class="bidding-form-title-row"><button class="back-link bidding-back-button" type="button" data-action="back"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 6l-6 6 6 6"></path><path d="M19 12H9"></path></svg><span>返回</span></button><h2>${title}</h2></div>
  ...
  function renderLimitForm() {
    ...
    const root = mount('添加限价', `
      <div class="page-card bidding-form-page" id="limitFormPage">
        <div class="bidding-form-title-row"><button class="back-link bidding-back-button" type="button" data-action="back"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 6l-6 6 6 6"></path><path d="M19 12H9"></path></svg><span>返回</span></button><h2>添加限价</h2></div>
        ...
        <div class="bidding-toolbar" style="margin-top:18px"><div class="bidding-toolbar-left"><button class="btn btn-primary btn-sm" type="button" data-action="batch-add">批量添加商品</button><button class="btn btn-sm" type="button" data-action="open-import">导入限价</button></div><span class="toolbar-note">设置执行周期后，商品限价将在竞价报价中生效</span></div>
      ...
      <div class="bidding-modal-mask product-picker-backdrop" id="batchModal"><div class="bidding-dialog wide product-picker-dialog"><div class="bidding-dialog-header product-picker-header"><h2>批量添加商品</h2><button class="bidding-dialog-close product-picker-close" type="button" data-action="close-modal" data-modal="batchModal" aria-label="关闭">×</button></div><div class="bidding-dialog-body product-picker-body"><div class="product-picker-filters"><label class="product-picker-filter"><span>商品名称</span><input data-batch-filter="keyword" placeholder="请输入商品名称/编号"></label><label class="product-picker-filter"><span>商品分类</span><select data-batch-filter="category"><option value="" disabled hidden selected>请选择商品分类</option>${service.categories.map((item) => `<option value="${esc(item)}">${esc(item)}</option>`).join('')}</select></label><div class="product-picker-filter-actions"><button class="btn btn-primary btn-sm" type="button" data-action="batch-query">查询</button><button class="btn btn-sm" type="button" data-action="batch-reset">重置</button></div></div><div class="product-picker-table-wrap"><table class="bidding-table product-picker-table"><thead><tr><th><input type="checkbox" data-action="batch-check-all"></th><th>图片</th><th>商品编码</th><th>商品名称</th><th>分类</th><th>计量单位</th><th>保质期单位</th><th>保质期</th></tr></thead><tbody id="batchProductBody"></tbody></table></div><div class="pagination bidding-batch-pagination product-picker-pagination"><span class="page-total product-picker-total">共 20 条数据</span><select class="page-size-select product-picker-page-size" aria-label="每页条数"><option>20 条/页</option></select><div class="page-btns product-picker-page-buttons"><button class="page-btn product-picker-page-button active" type="button" aria-current="page">1</button></div><div class="page-jump product-picker-page-jump"><span>跳至</span><input class="pagination-jump-input product-picker-jump-input" value="1" aria-label="跳转页码"><span>/ 1 页</span></div></div></div><div class="bidding-dialog-footer product-picker-footer"><button class="btn btn-sm" type="button" data-action="close-modal" data-modal="batchModal">关闭</button><button class="btn btn-primary btn-sm" type="button" data-action="confirm-batch">添加</button></div></div></div>
    ...
    function renderBatchRows() {
      ...
      root.querySelector('#batchProductBody').innerHTML = rows.length ? rows.map((item) => `<tr><td><input type="checkbox" data-batch-product="${item.id}"></td><td><span class="product-image-placeholder product-picker-image">图片</span></td><td>${esc(item.code)}</td><td class="product-picker-product" title="${esc(item.name)}">${esc(item.name)}</td><td>${esc(item.category)}</td><td>${esc(item.unit)}</td><td>${esc(item.shelfLifeUnit)}</td><td>${esc(item.shelfLife)}</td></tr>`).join('') : '<tr><td colspan="8" style="height:180px;color:#9aa4b2">暂无符合条件的商品</td></tr>';
      const total = root.querySelector('#batchModal .page-total');
      if (total) total.textContent = `共 ${rows.length} 条数据`;
    }
    renderDrafts();
    root.addEventListener('change', (event) => {
    ...
    const root = mount('废标管理', `
      <div class="page-card bidding-page" id="wastedManagementPage">
        <section class="bidding-filter-panel"><div class="bidding-filter-grid">
          ...
          <div class="bidding-filter-item"><label>标段</label><select data-filter="segment"><option value="" disabled hidden selected>请选择</option>${service.get('segments').map((item) => `<option value="${esc(item.name)}">${esc(item.name)}</option>`).join('')}</select></div>
        </div><div class="bidding-filter-actions"><button class="btn btn-primary btn-sm" type="button" data-action="query">查询</button><button class="btn btn-sm" type="button" data-action="reset">重置</button></div></section>
