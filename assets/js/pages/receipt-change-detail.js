(function () {
  ...
  window.OperationsService.get('receiptChanges', id).then((record) => {
    ...
    ${lines.map((line, index) => { const productDisplay = window.DomUtils.formatProductDisplay(line); return `<tr><td>${index + 1}</td><td><span class="goods-thumb">暂无图片</span></td><td class="goods-name-cell"><span class="product-display-text" title="${esc(productDisplay)}">${esc(productDisplay)}</span></td><td>${esc(line.unit)}</td><td>${line.shippingQty}</td><td>${line.acceptedQty}</td><td>${line.afterQty}</td><td>${line.differenceQty}</td><td>${money(line.unitPrice)}</td><td>${money(line.shippingAmount)}</td><td>${money(line.afterAmount)}</td><td>${money(line.differenceAmount)}</td><td>${esc(line.reason)}</td></tr>`; }).join('')}</tbody></table></div></section>`;
  });
})();
